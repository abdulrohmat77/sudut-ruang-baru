...
// Temporary shim: auth middleware removed during single-Supabase migration.
// Injects the CRM client + current display name so existing handlers continue to type-check.
// TODO: rewrite these handlers as plain async functions against pmis_* tables.
const requireSupabaseAuth = {
  id: "requireSupabaseAuth",
  client: async ({ next }: any) =>
    next({ context: { supabase: crmSupabase as any, userId: getCurrentUserName() as any, claims: {} as any } }),
...
async function audit(
  ...
  try {
    await sb.from("pmis_audit_logs").insert({ actor_id: userId, action, entity, entity_id: entity_id ?? null, meta });
  } catch {/* swallow */}
}
...
export const listAuditLogs = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    let q = context.supabase.from("pmis_audit_logs").select("*").order("created_at", { ascending: false }).limit(data.limit ?? 100);
    ...
    if (ids.length) {
      const { data: profs } = await context.supabase.from("pmis_profiles").select("id, full_name, email").in("id", ids as any);
...
export const listCorrespondence = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    let q = context.supabase.from("pmis_correspondence").select("*, pmis_projects(name, code), pmis_correspondence_templates(name)").order("updated_at", { ascending: false }).limit(200);
...
export const createCorrespondence = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: row, error } = await context.supabase.from("pmis_correspondence").insert(data).select("id").single();
    if (error) throw new Error(error.message);
...
export const updateCorrespondence = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { id, ...patch } = data;
    const { error } = await context.supabase.from("pmis_correspondence").update(patch).eq("id", id);
    if (error) throw new Error(error.message);
...
export const deleteCorrespondence = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("pmis_correspondence").delete().eq("id", data.id);
    if (error) throw new Error(error.message);
...
export const listCorrespondenceTemplates = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data, error } = await context.supabase.from("pmis_correspondence_templates").select("*").order("name");
    if (error) throw new Error(error.message);
    return { rows: data ?? [] };
  });
...
export const upsertCorrespondenceTemplate = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await context.supabase.from("pmis_correspondence_templates").upsert(payload);
    if (error) throw new Error(error.message);
...
export const deleteCorrespondenceTemplate = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: unknown) => z.object({ id: z.string().uuid() }).parse(d))
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("pmis_correspondence_templates").delete().eq("id", data.id);
    if (error) throw new Error(error.message);
...
export const listMeetings = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: unknown) => pid.parse(d))
  .handler(async ({ data, context }) => {
    let q = context.supabase.from("pmis_meetings").select("*, pmis_projects(name, code)").order("meeting_date", { ascending: false, nullsFirst: false }).limit(200);
    if (data.projectId) q = q.eq("project_id", data.projectId);
    const { data: rows, error } = await q;
    if (error) throw new Error(error.message);
    return { rows: rows ?? [] };
...
export const createMeeting = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const ai = (data.action_items ?? []).map((a: any) => ({ ...a, id: a.id ?? crypto.randomUUID(), done: a.done ?? false }));
    const { data: row, error } = await context.supabase.from("pmis_meetings").insert({ ...data, action_items: ai } as any).select("id").single();
    if (error) throw new Error(error.message);
...
export const updateMeeting = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await context.supabase.from("pmis_meetings").update(patch).eq("id", id);
    if (error) throw new Error(error.message);
...
export const deleteMeeting = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: unknown) => z.object({ id: z.string().uuid() }).parse(d))
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("pmis_meetings").delete().eq("id", data.id);
    if (error) throw new Error(error.message);
...
export const toggleMeetingAction = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: row, error: e1 } = await context.supabase.from("pmis_meetings").select("action_items").eq("id", data.meeting_id).single();
    ...
    const { error } = await context.supabase.from("pmis_meetings").update({ action_items: next }).eq("id", data.meeting_id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const listPhotos = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: unknown) => pid.parse(d))
  .handler(async ({ data, context }) => {
    let q = context.supabase.from("pmis_site_photos").select("*, pmis_projects(name, code)").order("taken_at", { ascending: false, nullsFirst: false }).limit(200);
    if (data.projectId) q = q.eq("project_id", data.projectId);
    const { data: rows, error } = await q;
    if (error) throw new Error(error.message);
    return { rows: rows ?? [] };
...
export const createPhoto = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("pmis_site_photos").insert(data);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const deletePhoto = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: unknown) => z.object({ id: z.string().uuid() }).parse(d))
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("pmis_site_photos").delete().eq("id", data.id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const updatePhoto = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { id, ...patch } = data;
    const { error } = await context.supabase.from("pmis_site_photos").update(patch).eq("id", id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const finalReport = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [proj, tasks, dailies, weeklies, monthlies, contracts, invoices, vos, qa, hse, risks, meetings, corr] = await Promise.all([
      sb.from("pmis_projects").select("*").eq("id", pid).single(),
      sb.from("pmis_project_tasks").select("weight, progress_percent, status").eq("project_id", pid),
      sb.from("pmis_daily_reports").select("id", { count: "exact", head: true }).eq("project_id", pid),
      sb.from("pmis_weekly_reports").select("planned_progress, actual_progress, variance").eq("project_id", pid),
      sb.from("pmis_monthly_reports").select("id", { count: "exact", head: true }).eq("project_id", pid),
      sb.from("pmis_contracts").select("contract_value, status").eq("project_id", pid),
      sb.from("pmis_invoices").select("amount, status").eq("project_id", pid),
      sb.from("pmis_variation_orders").select("impact_value, status").eq("project_id", pid),
      sb.from("pmis_qaqc_inspections").select("result").eq("project_id", pid),
      sb.from("pmis_hse_incidents").select("severity").eq("project_id", pid),
      sb.from("pmis_risks").select("probability, impact, status").eq("project_id", pid),
      sb.from("pmis_meetings").select("id, action_items").eq("project_id", pid),
      sb.from("pmis_correspondence").select("direction, status").eq("project_id", pid),
    ]);
    const ts = tasks.data ?? [];
    const totalWeight = ts.reduce((s: number, t: any) => s + Number(t.weight ?? 0), 0);
    const earned = ts.reduce((s: number, t: any) => s + (Number(t.weight ?? 0) * Number(t.progress_percent ?? 0) / 100), 0);
    const physicalProgress = totalWeight > 0 ? (earned / totalWeight) * 100 : 0;
    const wk = weeklies.data ?? [];
    const avgVariance = wk.length ? wk.reduce((s: number, w: any) => s + Number(w.variance ?? 0), 0) / wk.length : 0;
    const contractValue = (contracts.data ?? []).reduce((s: number, c: any) => s + Number(c.contract_value ?? 0), 0);
    const invPaid = (invoices.data ?? []).filter((i: any) => i.status === "paid").reduce((s: number, i: any) => s + Number(i.amount ?? 0), 0);
    const invOpen = (invoices.data ?? []).filter((i: any) => i.status !== "paid").reduce((s: number, i: any) => s + Number(i.amount ?? 0), 0);
    const voImpact = (vos.data ?? []).reduce((s: number, v: any) => s + Number(v.impact_value ?? 0), 0);
...
export const listUsers = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }) => {
    const { data: profiles, error } = await context.supabase.from("pmis_profiles").select("*").order("created_at", { ascending: false });
    if (error) throw new Error(error.message);
    const { data: roles } = await context.supabase.from("pmis_user_roles").select("user_id, role");
...
export const updateMyProfile = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("pmis_profiles").update(data).eq("id", context.userId);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const getMyProfile = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }) => {
    const { data, error } = await context.supabase.from("pmis_profiles").select("*").eq("id", context.userId).single();
    if (error) throw new Error(error.message);
    const { data: roles } = await context.supabase.from("pmis_user_roles").select("role").eq("user_id", context.userId);
...
export const assignRole = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("pmis_user_roles").insert({ ...data, assigned_by: context.userId });
    if (error) throw new Error(error.message);
...
export const revokeRole = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("pmis_user_roles").delete().eq("user_id", data.user_id).eq("role", data.role);
    if (error) throw new Error(error.message);
...
export const meetingSummary = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    let q = context.supabase.from("pmis_meetings").select("id, title, meeting_date, attendees, action_items").eq("project_id", data.projectId).order("meeting_date", { ascending: false });
    if (data.onDate) {
