...
export async function listTasks({ data }: { data: { projectId: string } }) {
  const projectId = z.string().uuid().parse(data.projectId);
  const { data: tasks, error } = await pmis("project_tasks")
...
export async function createTask({ data }: { data: unknown }) {
  const parsed = createSchema.parse(data);
  const { data: row, error } = await pmis("project_tasks").insert(parsed).select().single();
  if (error) throw new Error(error.message);
  return { task: row };
}
...
export async function updateTask({ data }: { data: unknown }) {
  const { id, ...rest } = updateSchema.parse(data);
  const { error } = await pmis("project_tasks").update(rest).eq("id", id);
  if (error) throw new Error(error.message);
  return { ok: true };
}
...
export async function deleteTask({ data }: { data: { id: string } }) {
  ...
  const { error } = await pmis("project_tasks").delete().eq("id", id);
  if (error) throw new Error(error.message);
  return { ok: true };
}

export async function sCurve({ data }: { data: { projectId: string } }) {
  const projectId = z.string().uuid().parse(data.projectId);
  const { data: tasks } = await pmis("project_tasks")
    .select("weight,progress_percent,planned_start,planned_end,actual_start,actual_end,status")
    .eq("project_id", projectId);
  const rows = ((tasks ?? []) as any[]).filter((t: any) => t.planned_start && t.planned_end);
  if (rows.length === 0) return { points: [] as { date: string; planned: number; actual: number | null }[] };
  const totalWeight = rows.reduce((s, t: any) => s + Number(t.weight ?? 0), 0) || 1;
